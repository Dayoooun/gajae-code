...
export function isReplayEligibleTeamNotification(
...
export function summarizeTeamNotifications(
  ...
  for (const notification of notifications) {
    ...
    if (isReplayEligibleTeamNotification(notification.delivery_state))
      summary.replay_eligible += 1;
  }
  return summary;
}
...
export async function createTeamMessageNotification(
  ...
  return runtime.writeNotification(dir, {
    id: runtime.messageNotificationId(
      teamName,
      message.to_worker,
      message.message_id,
    ),
...
export async function reconcileTeamNotifications(
  ...
  for (const recipient of [
    ...
    for (const message of (await runtime.readMailbox(dir, recipient))
      .messages) {
...
async function attemptConfiguredMailboxTransport(
  ...
  if (
    notification.kind !== "mailbox_message" ||
    notification.source.type !== "message"
  )
    return null;
  const message = (
    await runtime.readMailbox(dir, notification.recipient)
  ).messages.find(
  ...
  if (!result || (result.transport === "sdk" && result.state === "failed"))
    return null;
  return runtime.writeNotification(dir, {
    ...notification,
...
async function attemptPaneNotification(
  ...
  const transported = await attemptConfiguredMailboxTransport(
    runtime,
    dir,
    config,
    notification,
  ...
  const paneId =
    notification.recipient === "leader-fixed"
      ...
      : config.workers.find((worker) => worker.id === notification.recipient)
          ?.pane_id;
  let result: GjcTeamPaneAttemptResult = "deferred";
  let reason = "pane_missing";
  if (paneId) {
    if (
      config.tmux_session === "dry-run" ||
      env.GJC_TEAM_FAKE_PANE_ATTEMPT === "sent"
    ) {
      result = "sent";
...
export async function sendTeamMessage(
  ...
  const existing = (await runtime.listNotifications(dir)).some(
    (notification) =>
      notification.id ===
      runtime.messageNotificationId(
        config.team_name,
        toWorker,
        written.message_id,
      ),
  );
  const notification = await createTeamMessageNotification(
    runtime,
    dir,
    config.team_name,
    written,
...
export async function markTeamMailboxMessage(
  ...
  const existing =
    (await runtime.listNotifications(dir)).find(
  ...
  const state: GjcTeamNotificationDeliveryState =
    field === "delivered_at" ? "acknowledged" : "delivered";
