...
export type GjcTeamPhase =
  ...
  | "failed"
  | "cancelled";
export type GjcWorkerStatusState =
...
function mailboxMessageReceiptFields(
  teamName: string,
...
function notificationReceiptFields(
...
function notificationSummaryReceipt(
  ...
  return {
    team_name: teamName,
    notification_ids: result.notifications.map(
      (notification) => notification.id,
    ),
    delivery_states: result.notifications.map(
      (notification) => notification.delivery_state,
    ),
...
function normalizeGjcTeamWorkerCliMode(
  ...
  if (
    ...
    throw new Error(
      `Unsupported ${sourceEnv} value "${raw}". GJC team launches GJC teammate sessions only.`,
    );
  }
  throw new Error(`Invalid ${sourceEnv} value "${raw}". Expected: auto or gjc`);
}

export function resolveGjcTeamWorkerCli(
...
export function resolveGjcTeamWorkerCliPlan(
  ...
  const expanded =
    entries.length === 1
      ? Array.from({ length: workerCount }, () => entries[0] ?? "")
      : entries;
  return expanded.map((entry) => {
    const mode = normalizeGjcTeamWorkerCliMode(
      entry,
      GJC_TEAM_WORKER_CLI_MAP_ENV,
    );
    return mode === "auto" ? "gjc" : mode;
  });
}

export function translateGjcWorkerLaunchArgsForCli(
  workerCli: GjcTeamWorkerCli,
  args: string[],
): string[] {
  if (workerCli !== "gjc") {
    throw new Error(
      `Unsupported team worker CLI "${workerCli}". GJC team launches GJC teammate sessions only.`,
    );
  }
  return [...args];
}
...
function isGjcTeamTaskStatus(value: string): value is GjcTeamTaskStatus {
  return ["pending", "blocked", "in_progress", "completed", "failed"].includes(
    value,
  );
}

function parseGjcTeamTaskStatus(
  value: unknown,
  allowLegacyComplete = false,
): GjcTeamTaskStatus {
...
function stateWriterOptions(
  filePath: string,
  category: "state" | "ledger" | "report" | "prune",
  verb: string,
) {
  ...
  const sessionId =
    parts
      .map((part) => sessionIdFromDirName(part))
...
function mailboxPath(dir: string, worker: string): string {
  return path.join(
    dir,
    "mailbox",
    `${safePathSegment("worker_id", worker)}.json`,
  );
}
function mailboxDirPath(dir: string, worker: string): string {
  return path.join(dir, "mailbox", safePathSegment("worker_id", worker));
}
function mailboxMessagePath(
  dir: string,
  worker: string,
  messageId: string,
): string {
  return path.join(
    mailboxDirPath(dir, worker),
    `${safePathSegment("message_id", messageId)}.json`,
  );
}
function notificationPath(dir: string, notificationId: string): string {
  return path.join(
    dir,
    "notifications",
    `${safePathSegment("notification_id", notificationId)}.json`,
  );
}
...
function assertKnownWorker(
  ...
  if (!config.workers.some((candidate) => candidate.id === worker))
    throw new Error(`unknown_worker:${worker}`);
}
...
export function resolveGjcTeamStateRoot(
  cwd = process.cwd(),
  env: NodeJS.ProcessEnv = process.env,
): string {
...
function collectTeamGcWorkerPids(
  ...
  if (isPositivePid(lifecycle?.pid) && !pids.includes(lifecycle.pid))
    pids.push(lifecycle.pid);
  return pids;
}
 ...
 */
function classifyTeamGcWorkerPids(
  pids: number[],
  probe: GcPidProbe,
): TeamGcPidClassification {
...
function teamGcRecordDetail(
  ...
  return [
    ...
    ...(heartbeat
      ? [
          `heartbeat_alive=${heartbeat.alive}`,
...
/** @internal */
export async function listTeamWorkerGcRecords(
  teamRoot: string,
  probe: GcPidProbe,
): Promise<GcRecord[]> {
  ...
  for (const teamEntry of teamEntries) {
    ...
    for (const workerEntry of workerEntries) {
      ...
      try {
        heartbeat = await readJsonFile<WorkerHeartbeatFile>(
          path.join(dir, "heartbeat.json"),
        );
        lifecycle = await readJsonFile<GjcTeamWorkerLifecycle>(
...
/** @internal */
export async function pruneTeamWorkerGcRecord(
  record: GcRecord,
  probe: GcPidProbe,
): Promise<boolean> {
  ...
  const heartbeat = await readJsonFile<WorkerHeartbeatFile>(
    path.join(record.path, "heartbeat.json"),
  );
  const lifecycle = await readJsonFile<GjcTeamWorkerLifecycle>(
    path.join(record.path, "lifecycle.json"),
  );
  const pids = collectTeamGcWorkerPids(heartbeat, lifecycle);
  ...
  try {
    for (const entry of await fs.readdir(claimDir, { withFileTypes: true })) {
      ...
      await removeFileAudited(
        claimPath,
        stateWriterOptions(claimPath, "prune", "gc-team-worker"),
      );
    }
  } catch (error) {
...
function stateCategoryForJsonPath(filePath: string): "state" | "ledger" {
  return filePath.endsWith(".jsonl") ||
    filePath.includes(`${path.sep}telemetry${path.sep}`)
    ? "ledger"
    : "state";
}
...
async function writeJsonFile(filePath: string, value: unknown): Promise<void> {
  await writeJsonAtomic(
    filePath,
    value,
    stateWriterOptions(filePath, stateCategoryForJsonPath(filePath), "write"),
  );
}
async function writeJsonFileNoClobber(
  filePath: string,
  value: unknown,
): Promise<boolean> {
  try {
    await createJsonNoClobber(
      ...
      stateWriterOptions(
        filePath,
        stateCategoryForJsonPath(filePath),
        "create",
      ),
    );
    return true;
...
async function appendJsonl(filePath: string, value: unknown): Promise<void> {
  await appendJsonlAudited(
    filePath,
    value,
    stateWriterOptions(filePath, "ledger", "append"),
  );
}
...
function evidenceRefsForEvent(event: GjcTeamEvent): string[] | undefined {
  ...
  if (
    ...
    event.data &&
    "completion_evidence" in event.data
  )
    refs.push(`task:${event.task_id}:completion_evidence`);
  if (event.task_id && event.type === "task_claim_recovered")
    refs.push(`task:${event.task_id}:claim_recovery`);
  if (event.worker && event.type.startsWith("worker_"))
    refs.push(`worker:${event.worker}`);
...
function messageBodyTraceProjection(
...
function traceDataForEvent(
  ...
  switch (event.type) {
    case "message_sent": {
      ...
      Object.assign(
        data,
        messageBodyTraceProjection(pickString(event.message)),
      );
      break;
    }
    ...
    case "worker_lifecycle_nudge": {
      ...
      setIfDefined(
        data,
        "auto_action_taken",
        pickBoolean(source.auto_action_taken),
      );
      break;
    }
    case "team_shutdown": {
      ...
      setIfDefined(
        data,
        "shutdown_request_id",
        pickString(source.shutdown_request_id),
      );
      setIfDefined(
        data,
        "graceful_shutdown_complete",
        pickBoolean(source.graceful_shutdown_complete),
      );
      if (Array.isArray(source.evidence_failures))
        data.evidence_failure_count = source.evidence_failures.length;
      break;
    }
...
async function appendTraceForEvent(
  dir: string,
  event: GjcTeamEvent,
): Promise<void> {
...
async function appendEvent(
  dir: string,
  event: Omit<GjcTeamEvent, "ts" | "event_id">,
...
async function readConfig(dir: string): Promise<GjcTeamConfig> {
  const config = await readJsonFile<GjcTeamConfig>(
    path.join(dir, "config.json"),
  );
  if (!config) throw new Error(`team_config_not_found:${dir}`);
  const tmuxSessionName =
    config.tmux_session_name ?? config.tmux_session?.split(":")[0] ?? "";
  return {
    ...config,
    ...
    worker_cli_plan:
      config.worker_cli_plan ??
      Array.from({ length: config.worker_count }, () => "gjc"),
  };
}
...
const workerIntegrationConfigCache = new Map<
  string,
  WorkerIntegrationConfigCacheEntry
>();

async function readConfigForWorkerIntegration(
  dir: string,
): Promise<GjcTeamConfig> {
  ...
  if (
    ...
    nowMs - cached.checkedAt <= WORKER_INTEGRATION_CONFIG_CACHE_TTL_MS
  )
...
async function readPhase(dir: string): Promise<GjcTeamPhase> {
  const phase = await readJsonFile<{ current_phase?: GjcTeamPhase }>(
...
function parseGjcWorkerStatusState(value: unknown): GjcWorkerStatusState {
  return typeof value === "string" && isGjcWorkerStatusState(value)
    ? value
    : "unknown";
}
function parseRequiredGjcWorkerStatusState(
  value: unknown,
...
function lifecycleStateForWorkerStatus(
...
function isGjcTeamWorkerLifecycleState(
  value: string,
): value is GjcTeamWorkerLifecycleState {
  return [
    "starting",
    "ready",
    "working",
...
function parseGjcTeamWorkerLifecycleState(
  value: unknown,
): GjcTeamWorkerLifecycleState {
  return typeof value === "string" && isGjcTeamWorkerLifecycleState(value)
    ? value
    : "unknown";
}
...
async function readWorkerStatusFile(
  ...
  return (
    (await readJsonFile<WorkerStatusFile>(
      path.join(workerDir(dir, worker), "status.json"),
    )) ?? {
      state: "unknown",
      updated_at: now(),
    }
...
async function readWorkerLifecycleRecord(
  dir: string,
  worker: GjcTeamWorker,
): Promise<GjcTeamWorkerLifecycle> {
  const workerStatus = await readWorkerStatusFile(dir, worker.id);
  const heartbeat = await readJsonFile<WorkerHeartbeatFile>(
    path.join(workerDir(dir, worker.id), "heartbeat.json"),
  );
  const rawLifecycle = await readJsonFile<Partial<GjcTeamWorkerLifecycle>>(
    workerLifecyclePath(dir, worker.id),
  );
  ...
  const lifecycle: GjcTeamWorkerLifecycle = {
    worker: worker.id,
    lifecycle_state: parseGjcTeamWorkerLifecycleState(
      rawLifecycle?.lifecycle_state,
    ),
  ...
  if (rawLifecycle?.stop_reason)
    lifecycle.stop_reason = rawLifecycle.stop_reason;
  if (rawLifecycle?.shutdown_request_id)
    lifecycle.shutdown_request_id = rawLifecycle.shutdown_request_id;
  if (rawLifecycle?.shutdown_requested_at)
    lifecycle.shutdown_requested_at = rawLifecycle.shutdown_requested_at;
  if (
  ...
  if (typeof shutdownAck?.status === "string")
    lifecycle.shutdown_ack_status = shutdownAck.status;
  return lifecycle;
}
...
async function readWorkerLifecycleById(
  ...
  const entries = await Promise.all(
    config.workers.map((worker) => readWorkerLifecycleRecord(dir, worker)),
  );
...
async function writeWorkerLifecycleForConfig(
  ...
  updatesFor: (
    worker: GjcTeamWorker,
  ) => Partial<GjcTeamWorkerLifecycle> = () => ({}),
): Promise<Record<string, GjcTeamWorkerLifecycle>> {
  const entries = await Promise.all(
    config.workers.map((worker) =>
      writeWorkerLifecycleRecord(
        dir,
        worker,
        lifecycleState,
        updatesFor(worker),
...
export async function persistGjcTeamModeStateSummary(
  snapshot: GjcTeamSnapshot,
  cwd = process.cwd(),
): Promise<void> {
  const active =
    snapshot.phase !== "complete" && snapshot.phase !== "cancelled";
  const updatedAt = now();
...
function readClaimRecord(value: unknown): GjcTeamTaskClaim | undefined {
  ...
  const leasedUntil =
    typeof value.leased_until === "string" ? value.leased_until : "";
...
function isWorkerHeartbeatStale(
  ...
  const thresholdMs = parseDurationEnv(
    env,
    "GJC_TEAM_HEARTBEAT_STALE_MS",
    120_000,
  );
  if (thresholdMs <= 0) return false;
  const heartbeatAt = Date.parse(
    heartbeat?.last_turn_at ?? worker.last_heartbeat,
  );
  return (
    Number.isFinite(heartbeatAt) && Date.now() - heartbeatAt >= thresholdMs
  );
}
...
async function detectGjcTeamWorkerLivenessReasons(
  ...
  if (lifecycle.lifecycle_state === "failed")
    appendLivenessRecoveryReason(reasons, "worker_lifecycle_failed");
  if (lifecycle.lifecycle_state === "stopped")
    appendLivenessRecoveryReason(reasons, "worker_lifecycle_stopped");
  if (isWorkerHeartbeatStale(worker, heartbeat, env))
    appendLivenessRecoveryReason(reasons, "stale_heartbeat");
  if (
    !config.dry_run &&
    (!worker.pane_id?.startsWith("%") ||
...
async function reconcileGjcTeamStaleClaims(
  ...
  for (const worker of config.workers) {
    const reasons = await detectGjcTeamWorkerLivenessReasons(
      dir,
      config,
      worker,
      env,
    ...
    if (
      reasons.includes("missing_pane") &&
      reasons.includes("worker_lifecycle_stopped") === false
    ) {
  ...
  for (const task of await readTasks(dir)) {
    ...
    if (isPastTimestamp(claim.leased_until))
      appendLivenessRecoveryReason(reasons, "claim_expired");
    if (reasons.length === 0) continue;
...
const GJC_TEAM_INTEGRATION_ATTENTION_STATUSES =
  new Set<GjcTeamIntegrationStatus>([
    "integration_failed",
    "merge_conflict",
    "cherry_pick_conflict",
    "rebase_conflict",
  ]);
const GJC_TEAM_INTEGRATION_SETTLED_STATUSES = new Set<GjcTeamIntegrationStatus>(
  ["idle", "integrated"],
);
...
async function hasPendingGjcTeamIntegration(
  ...
  for (const worker of config.workers) {
    ...
    if (
      integration?.status &&
      GJC_TEAM_INTEGRATION_ATTENTION_STATUSES.has(integration.status)
    )
      return true;

    const request = await readJsonFile<GjcWorkerIntegrationDedupeState>(
      workerIntegrationDedupePath(dir, worker.id),
    );
    if (!request?.last_requested_at) continue;
    if (!integration?.status || !integration.updated_at) return true;
    if (GJC_TEAM_INTEGRATION_ATTENTION_STATUSES.has(integration.status))
      return true;
    if (
...
async function resolveGjcTeamSnapshotPhase(
  ...
  if (tasks.length === 0 || !tasks.every(isGjcTeamTaskCompletionVerified))
    return storedPhase;
  return (await hasPendingGjcTeamIntegration(dir, config, monitor))
    ? "awaiting_integration"
    : storedPhase;
}
...
async function findTeamDir(
  ...
  if (await readJsonFile<GjcTeamConfig>(path.join(exact, "config.json")))
    return exact;
  const candidates = await listGjcTeams(cwd, env);
  const input = sanitizeName(teamName);
  const matches = candidates.filter(
    (candidate) =>
      candidate.team_name === input ||
      sanitizeName(candidate.display_name) === input,
  );
  if (matches.length === 1) return matches[0].state_dir;
  if (matches.length > 1)
    throw new Error(
      `ambiguous_team_name:${teamName}:${matches.map((match) => match.team_name).join(",")}`,
    );
...
async function runGitResultAsync(
  cwd: string,
  args: string[],
): Promise<GitResult> {
...
function parseWorktreeMode(args: string[]): {
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    if (arg.startsWith("--worktree=")) {
      ...
      mode = name
        ? { enabled: true, detached: false, name }
        : { enabled: true, detached: true, name: null };
      continue;
    }
    if (arg.startsWith("-w=") || (arg.startsWith("-w") && arg.length > 2)) {
      const name = arg.startsWith("-w=")
        ? arg.slice("-w=".length).trim()
        : arg.slice(2).trim();
      mode = name
        ? { enabled: true, detached: false, name }
        : { enabled: true, detached: true, name: null };
...
function resolveDefaultWorktreeMode(
  mode?: GjcTeamWorktreeMode,
): GjcTeamWorktreeMode {
...
function findWorktreePath(
  ...
  for (const line of raw.split(/\r?\n/))
    if (
      ...
      path.resolve(line.slice("worktree ".length)) === resolved
    )
      return resolved;
  return null;
}
export function resolveWorkerWorktreePath(input: {
  ...
  if (input.platform === "win32" && input.isPsmux) {
    const slug = stableHash(
      [input.repoRoot, input.stateDir, input.teamName].join("\0"),
    ).slice(0, 12);
...
async function ensureWorkerWorktree(
  ...
  if (!isGitRepository(cwd))
    throw new Error(`team_worktree_requires_git_repo:${cwd}`);
  ...
  if (existing) {
    if (worktreeIsDirty(worktreePath))
      throw new Error(`worktree_dirty:${worktreePath}`);
    if (mode.detached && worktreeHead(worktreePath) !== baseRef)
      throw new Error(`worktree_stale:${worktreePath}`);
  } else {
...
function readGjcTmuxProfileValue(
  tmuxCommand: string,
  sessionName: string,
): string {
...
function tagTmuxSessionAsGjcLeader(
  tmuxCommand: string,
  sessionName: string,
): boolean {
  const result = Bun.spawnSync(
...
function readCurrentTmuxLeaderContext(
  ...
  if (Bun.which(tmuxCommand) === null)
    throw new Error(
      buildTeamTmuxLeaderRequirementMessage(
        `tmux_not_installed:${tmuxCommand}`,
      ),
    );
  ...
  const displayTarget = activeSession
    ? buildGjcTmuxExactOptionTarget(activeSession, { env })
    : env.TMUX_PANE?.trim();
  ...
  if (result.exitCode !== 0) {
    ...
    const insideTmux = Boolean(
      env.TMUX?.trim() || env.TMUX_PANE?.trim() || activeSession,
    );
    const stderr = result.stderr.toString().trim();
    throw new Error(
      buildTeamTmuxLeaderRequirementMessage(
        insideTmux
          ? `tmux_query_failed${stderr ? `:${stderr}` : ""}`
          : "not_inside_tmux",
      ),
    );
  }
  const [sessionAndWindow = "", leaderPaneId = ""] = result.stdout
    .toString()
    .trim()
    .split(/\s+/);
  ...
  if (!sessionName || !windowIndex || !leaderPaneId.startsWith("%"))
    throw new Error(
      buildTeamTmuxLeaderRequirementMessage(
        `invalid_tmux_context:${result.stdout.toString().trim()}`,
      ),
    );
  if (
    readGjcTmuxProfileValue(tmuxCommand, sessionName) !== GJC_TMUX_PROFILE_VALUE
  ) {
    ...
    if (
      !tagged ||
      readGjcTmuxProfileValue(tmuxCommand, sessionName) !==
        GJC_TMUX_PROFILE_VALUE
    )
      throw new Error(
        buildTeamTmuxLeaderRequirementMessage(
...
function isAbsoluteRealPath(
  ...
  return Boolean(
    normalized &&
    !isBunVirtualPath(normalized) &&
    pathModule.isAbsolute(normalized),
  );
}
function isGjcExecutablePath(
  candidate: string | undefined,
  pathModule: typeof path,
): candidate is string {
  return (
    isAbsoluteRealPath(candidate, pathModule) &&
    /^gjc(?:[._-]|$)/i.test(pathModule.basename(candidate.trim()))
  );
}

function formatWorkerExecutable(
  platform: NodeJS.Platform,
  executable: string,
): string {
...
export function resolveGjcWorkerCommand(
  ...
  const sourceEntrypoint =
    entrypoint &&
    !isBunVirtualPath(entrypoint) &&
    /\.(?:[cm]?[jt]s)$/i.test(entrypoint);
  ...
  if (sourceEntrypoint) {
    ...
    const absoluteEntrypoint = pathModule.isAbsolute(entrypoint)
      ? entrypoint
      : pathModule.resolve(cwd, entrypoint);
    if (!isAbsoluteRealPath(absoluteEntrypoint, pathModule))
      throw unresolvedWorkerAuthorityError();
...
export function buildWorkerCommand(
  ...
  const envLines = [
    ...
    envAssignment(
      "GJC_TEAM_INTERNAL_WORKER",
      `${config.team_name}/${worker.id}`,
    ),
    ...
    envAssignment(
      ...
      config.leader.session_id.trim() || config.team_name,
    ),
    ...(worker.worktree_path
      ? [envAssignment("GJC_TEAM_WORKTREE_PATH", worker.worktree_path)]
      : []),
  ];
...
function shouldDispatchWorkerWithSendKeys(
  tmuxCommand: string,
  platform: NodeJS.Platform = process.platform,
): boolean {
  return (
    platform === "win32" || path.basename(tmuxCommand).toLowerCase() === "psmux"
  );
}
...
function parseExplicitTeamLanes(task: string): GjcTeamInitialLane[] {
  ...
  const boundaryHeading =
    /^#{1,6}\s+(?:Integration Owner|Verification Plan|ADR|Approval State)\b/i;
...
function hasAmbiguousLaneSplitIntent(task: string): boolean {
  return (
    /\bsplit\s+lanes?\s*:/i.test(task) ||
    /\blanes?\s*:\s*[A-Z]\b/i.test(task) ||
    /\bLane\s+[A-Z]\s*[—–-]/.test(task)
  );
}
...
function buildInitialTasks(
  ...
  if (lanes.length > 0)
    return lanes.map((lane, index) => {
      ...
      const objective = [`${laneTitle}`, lane.body]
    ...
    for (const worker of config.workers) {
      ...
      const splitTarget: string =
        worker.index === 1
          ...
          : (rightStackRootPaneId ?? config.leader.pane_id);
      ...
      if (split.exitCode !== 0)
        throw new Error(
          split.stderr.toString().trim() ||
            `tmux_split_failed:${config.tmux_target}:${worker.id}`,
        );
      const paneId: string =
        split.stdout.toString().trim().split(/\r?\n/)[0]?.trim() ?? "";
      if (!paneId.startsWith("%"))
        throw new Error(
          `tmux_split_missing_pane:${config.tmux_target}:${worker.id}`,
        );
      ...
      if (useSendKeysFallback) {
        ...
        Bun.spawnSync(
          [config.tmux_command, "send-keys", "-l", "-t", paneId, workerCommand],
          {
            stdout: "ignore",
            stderr: "ignore",
          },
        ...
        const sendKeys = Bun.spawnSync(
          [config.tmux_command, "send-keys", "-t", paneId, "Enter"],
          {
            stdout: "ignore",
            stderr: "ignore",
          },
...
function paneBelongsToTeamTarget(
  config: GjcTeamConfig,
  paneId: string,
): boolean {
  ...
  const [target = "", detectedPaneId = ""] = result.stdout
    .toString()
    .trim()
    .split(/\s+/);
...
async function rollbackCreatedWorktrees(
  ...
  for (const worker of workers
    .filter((worker) => worker.worktree_created)
    .reverse())
    if (worker.worktree_repo_root && worker.worktree_path)
      Bun.spawnSync(
        ["git", "worktree", "remove", "--force", worker.worktree_path],
        {
...
async function removeCleanCreatedWorktrees(
  workers: GjcTeamWorker[],
): Promise<void> {
  for (const worker of workers
    .filter((worker) => worker.worktree_created)
...
async function listConflictFilesAsync(cwd: string): Promise<string[]> {
  const result = await runGitResultAsync(cwd, [
    "diff",
    "--name-only",
    "--diff-filter=U",
  ]);
...
const UNMERGED_GIT_STATUS_CODES = new Set([
  "DD",
  "AU",
  "UD",
  "UA",
  ...
  for (const file of files) {
    ...
    const isProtected = PROTECTED_WORKER_CHECKPOINT_PREFIXES.some((prefix) => {
      ...
      return (
        ...
        normalized.slice(head.length).includes(tail)
      );
    });
...
export function classifyWorkerCheckpointStatus(
  cwd: string,
): GjcWorkerCheckpointClassification {
...
export async function classifyWorkerCheckpointStatusAsync(
  ...
  const status = await runGitResultAsync(cwd, [
    "status",
    "--porcelain",
    "-uall",
  ]);
...
async function notifyIntegrationConflict(
  ...
  await Promise.all([
    notifyLeader(config, worker, body, cwd, env),
    notifyWorker(config, worker, body, cwd, env),
  ]);
}
...
function workerMergeRef(worker: GjcTeamWorker, workerHead: string): string {
  ...
  const branch = tryRunGit(worker.worktree_path, [
    "rev-parse",
    "--abbrev-ref",
    "HEAD",
  ]);
...
async function integrateGjcWorkerCommits(
  ...
  for (const worker of config.workers) {
    ...
    const state: GjcTeamWorkerIntegrationState = {
      ...
      last_leader_head:
        leaderHead ?? integrationByWorker[worker.id]?.last_leader_head,
    };
    ...
    if (isAncestor(worker.worktree_path, leaderHead, workerHead)) {
      ...
      if (merge.ok) {
        ...
        if (
          newLeaderHead &&
          newLeaderHead !== leaderHead &&
          isAncestor(leaderCwd, workerHead, "HEAD")
        ) {
          integrationByWorker[worker.id] = {
            ...state,
            last_integrated_head: workerHead,
          ...
          hygieneEntries.push({
            ...
            worktree_path: worker.worktree_path,
            detail:
              "Runtime merge succeeded but did not advance the leader head.",
          });
          await notifyLeader(
            config,
            worker,
    ...
    const baseline =
      ...
      tryRunGit(worker.worktree_path, [
        "rev-parse",
        "--verify",
        state.last_integrated_head,
      ])
    ...
    for (const commit of commits) {
      const pick = runGitResult(leaderCwd, [
        "cherry-pick",
        "--allow-empty",
        commit,
      ]);
  ...
  if (cycleLeaderHead && newLeaderHead && cycleLeaderHead !== newLeaderHead) {
    for (const worker of config.workers) {
      if (!worker.worktree_path || !(await pathExists(worker.worktree_path)))
        continue;
      const status = await readGjcWorkerStatus(
        config.team_name,
        worker.id,
        cwd,
        env,
      ...
      if (worktreeIsDirty(worker.worktree_path)) {
        hygieneEntries.push({
          ...
          worktree_path: worker.worktree_path,
          detail:
            "Worker worktree is dirty after integration; automatic cross-rebase skipped.",
        });
        continue;
      }
      const before = resolveHead(worker.worktree_path);
      const rebase = runGitResult(worker.worktree_path, [
        "rebase",
        newLeaderHead,
      ]);
      if (rebase.ok) {
        ...
        hygieneEntries.push({
          ...
          worktree_path: worker.worktree_path,
          detail:
            "Runtime rebase moved worker history onto updated leader head.",
        });
      } else {
...
export async function startGjcTeam(
  ...
  if (
    ...
    options.workerCount > GJC_TEAM_MAX_WORKERS
  )
    throw new Error(
      `invalid_team_worker_count:${options.workerCount}:expected_1_${GJC_TEAM_MAX_WORKERS}`,
    );
  ...
  const teamName = sanitizeName(
    options.teamName ?? makeTeamName(options.task, env),
  );
  const displayName =
    sanitizeName(options.teamName ?? options.task).slice(0, 30) || teamName;
  ...
  const initialWorkers = buildWorkers(
    options.workerCount,
    options.agentType,
    stateRoot,
  );
  ...
  await appendTelemetry(dir, {
    type: "team_runtime",
    message: options.dryRun
      ? "Native gjc team dry-run state initialized"
      : "Native gjc team runtime initialized",
    data: {
      state_root: stateRoot,
      worker_command: config.worker_command,
      worker_cli_plan: workerCliPlan,
  ...
  try {
    tmuxWorkers = await startTmuxSession(
      config,
      dir,
  ...
  await writeWorkerLifecycleForConfig(
    dir,
    runningConfig,
    "starting",
    (worker) => ({
...
export async function readGjcTeamSnapshot(
  ...
  const monitor = await readJsonFile<GjcTeamMonitorSnapshot>(
    monitorSnapshotPath(dir),
  );
  ...
  const phase = await resolveGjcTeamSnapshotPhase(
    dir,
    config,
    storedPhase,
    tasks,
...
function workerIntegrationFingerprint(
  head: string | null,
  classification: GjcWorkerCheckpointClassification,
): string {
...
export async function requestGjcWorkerIntegrationAttempt(
  ...
  const worker =
    env.GJC_TEAM_WORKER_ID?.trim() ||
    env.GJC_TEAM_INTERNAL_WORKER?.split("/").pop()?.trim();
  if (!teamName || !worker) return { requested: false, reason: "not_worker" };
  const dir = await findTeamDir(teamName, cwd, env);
  const config = await readConfigForWorkerIntegration(dir);
  const configuredWorker = config.workers.find(
    (candidate) => candidate.id === worker,
  );
  const worktreePath =
    env.GJC_TEAM_WORKTREE_PATH?.trim() || configuredWorker?.worktree_path;
  ...
  if (
    ...
    configuredWorker?.worktree_base_ref === head
  ) {
    return {
      requested: false,
      reason: "no_changes",
  ...
  const dedupe =
    (await readJsonFile<GjcWorkerIntegrationDedupeState>(dedupePath)) ?? {};
  ...
  await appendCommitHygieneEntries(config, [
    {
      ...
      detail:
        "Worker turn-end requested a leader integration attempt for semantic git changes.",
    },
  ]);
  return {
    requested: true,
...
export async function monitorGjcTeam(
  ...
  const dir = await findTeamDir(teamName, cwd, env);
  const config = await readConfig(dir);
  const previous = await readJsonFile<GjcTeamMonitorSnapshot>(
    monitorSnapshotPath(dir),
  );
  await reconcileGjcTeamStaleClaims(teamName, dir, config, env);
  const integrationByWorker = await integrateGjcWorkerCommits(
    config,
    dir,
    previous,
    cwd,
  ...
  try {
    ...
    const snapshots = await Promise.all(
      entries
        ...
        .map((entry) =>
          readGjcTeamSnapshot(entry.name, cwd, env).catch(() => null),
        ),
    );
    return snapshots.filter(
      (snapshot): snapshot is GjcTeamSnapshot => snapshot != null,
    );
  } catch (error) {
...
async function writeGjcWorkerStartupAck(
  ...
  await writeJsonFile(
    path.join(workerDir(dir, worker), "startup-ack.json"),
    ack,
  );
...
async function writeLifecycleNudge(
  ...
  const nudgePath = path.join(
    workerDir(dir, worker),
    "nudges",
    `${fingerprint}.json`,
  );
  const existing = await readJsonFile<Record<string, unknown>>(nudgePath);
  const nowMs = Date.now();
  const cooldownMs = parseDurationEnv(
    env,
    "GJC_TEAM_NUDGE_COOLDOWN_MS",
    30_000,
  );
  const cooldownUntil =
    typeof existing?.cooldown_until === "string"
      ? Date.parse(existing.cooldown_until)
      : 0;
  if (existing && Number.isFinite(cooldownUntil) && cooldownUntil > nowMs)
    return;
  const firstSeen =
...
async function computeLifecycleNudges(
  ...
  const startupGraceMs = parseDurationEnv(
    env,
    "GJC_TEAM_STARTUP_GRACE_MS",
    30_000,
  );
  const heartbeatStaleMs = parseDurationEnv(
    env,
    "GJC_TEAM_HEARTBEAT_STALE_MS",
    120_000,
  );
  ...
  const ageMs =
    Date.now() - (Number.isFinite(createdAt) ? createdAt : Date.now());
  for (const worker of config.workers) {
    const ack = await readJsonFile<Record<string, unknown>>(
      path.join(workerDir(dir, worker.id), "startup-ack.json"),
    );
    ...
    const heartbeat = await readGjcWorkerHeartbeat(
      config.team_name,
      worker.id,
      config.leader.cwd,
      {
    ...
    if (
      ...
      Date.now() - heartbeatAt >= heartbeatStaleMs
    ) {
      await writeLifecycleNudge(
        dir,
        worker.id,
...
export async function shutdownGjcTeam(
    ...
    .map((task) => {
      const reason =
        task.status === "completed"
          ? getGjcTeamTaskCompletionEvidenceFailure(task)
          : null;
      return reason ? { task_id: task.id, reason } : null;
    })
    .filter(
      (failure): failure is { task_id: string; reason: string } =>
        failure != null,
    );
  ...
  const pendingIntegration = completionVerified
    ? await hasPendingGjcTeamIntegration(dir, config, monitor)
    : false;
  killWorkerPanes(config);
  ...
  if (evidenceFailures.length > 0)
    shutdownData.evidence_failures = evidenceFailures;
  await appendEvent(dir, {
    type: "team_shutdown",
    message:
...
export async function updateGjcTeamTask(
  ...
  return taskStore(await findTeamDir(teamName, cwd, env)).update(
    taskId,
    updates,
  );
}
export async function claimGjcTeamTask(
  ...
  const recovered = await reconcileGjcTeamStaleClaims(
    teamName,
    dir,
    config,
    env,
...
export async function transitionGjcTeamTaskStatus(
  ...
  return taskStore(dir).transition(
    taskId,
    status,
    claimToken,
    workerId,
...
export async function releaseGjcTeamTaskClaim(
  ...
  return taskStore(await findTeamDir(teamName, cwd, env)).release(
    taskId,
    claimToken,
    workerId,
  );
...
async function listNotificationRecords(
  dir: string,
  ...
  try {
    ...
    const records = await Promise.all(
      entries
        ...
        .map((entry) =>
          readJsonFile<GjcTeamNotification>(
            path.join(notificationsDir, entry.name),
          ),
        ),
    );
    return records
...
async function readNotificationRecord(
  ...
  const notification = await readJsonFile<GjcTeamNotification>(
    notificationPath(dir, notificationId),
  );
  if (!notification)
    throw new Error(`notification_not_found:${notificationId}`);
  return notification;
}
...
async function writeNotificationRecord(
  ...
  const existing = await readJsonFile<GjcTeamNotification>(
    notificationPath(dir, notification.id),
  );
  const merged: GjcTeamNotification = existing
    ? {
        ...existing,
        ...notification,
        delivery_state: mergeNotificationState(
          existing.delivery_state,
          notification.delivery_state,
        ),
        created_at: existing.created_at,
        replay_count: Math.max(
          existing.replay_count ?? 0,
          notification.replay_count ?? 0,
        ),
        updated_at: now(),
      }
    : notification;
...
async function readLegacyMailbox(
  dir: string,
  worker: string,
): Promise<{ messages: GjcTeamMailboxMessage[] }> {
  return (
    (await readJsonFile<{ messages: GjcTeamMailboxMessage[] }>(
      mailboxPath(dir, worker),
    )) ?? { messages: [] }
  );
}
async function readMailbox(
  ...
  for (const message of (await readLegacyMailbox(dir, worker)).messages ?? [])
  ...
  try {
    ...
    const records = await Promise.all(
      entries
        .filter((entry) => entry.isFile() && entry.name.endsWith(".json"))
        .map((entry) =>
          readJsonFile<GjcTeamMailboxMessage>(
            path.join(mailboxDirPath(dir, worker), entry.name),
          ),
        ),
    );
...
async function writeLegacyMailboxView(
  dir: string,
  worker: string,
): Promise<void> {
...
export async function replayGjcTeamNotifications(
  ...
  return replayTeamNotifications(
...
export async function updateGjcWorkerStatus(
  ...
  const lifecycleState =
    currentLifecycle.lifecycle_state === "stopped"
      ? "stopped"
      : lifecycleStateForWorkerStatus(status);
...
export async function readGjcWorkerHeartbeat(
  ...
  return readJsonFile<WorkerHeartbeatFile>(
    path.join(workerDir(dir, worker), "heartbeat.json"),
  );
}
...
export async function writeGjcWorkerInbox(
  ...
  await writeReport(
    filePath,
    content,
    stateWriterOptions(filePath, "report", "write"),
  );
...
export async function writeGjcWorkerIdentity(
  ...
  await writeJsonFile(
    path.join(workerDir(dir, worker.id), "identity.json"),
    worker,
  );
  return worker;
}
...
export async function writeGjcMonitorSnapshot(
  ...
  await writeJsonFile(
    monitorSnapshotPath(await findTeamDir(teamName, cwd, env)),
    snapshot,
  );
  return snapshot;
}
...
export async function readGjcShutdownAck(
  ...
  assertKnownWorker(config, worker);
  return readJsonFile<Record<string, unknown>>(
    path.join(workerDir(dir, worker), "shutdown-ack.json"),
  );
}
...
export async function executeGjcTeamApiOperation(
  ...
  switch (operation) {
    ...
    case "notification-replay":
      return notificationSummaryReceipt(
        teamName,
        await replayGjcTeamNotifications(teamName, cwd, env),
      );
    case "notification-mark-pane-attempt": {
      const dir = await findTeamDir(teamName, cwd, env);
      const notification = await readNotificationRecord(
        dir,
        String(input.notification_id),
      );
      const updated = await writeNotificationRecord(dir, {
        ...notification,
        delivery_state: parsePaneAttemptResult(
          String(input.result ?? "failed"),
        ),
        pane_attempt_result: parsePaneAttemptResult(
          String(input.result ?? "failed"),
        ),
    ...
    case "read-manifest":
      return readJsonFile(
        path.join(await findTeamDir(teamName, cwd, env), "manifest.v2.json"),
      );
    ...
    case "write-worker-identity":
      return writeGjcWorkerIdentity(
        teamName,
        {
          ...
          status: "idle",
          last_heartbeat: now(),
          assigned_tasks: Array.isArray(input.assigned_tasks)
            ? input.assigned_tasks.map(String)
            : [],
        },
        cwd,
        env,
    ...
    case "write-shutdown-request": {
      ...
      return writeGjcShutdownRequest(
        ...
        cwd,
        env,
        typeof shutdownRequestIdInput === "string"
          ? shutdownRequestIdInput
          : undefined,
...
export function parseTeamLaunchArgs(argv: string[]): GjcTeamStartOptions {
  ...
  const positionals = parsedWorktree.remainingArgs.filter(
    (arg) => !arg.startsWith("--"),
  );
  ...
  if (
    ...
    workerCount < 1 ||
    workerCount > GJC_TEAM_MAX_WORKERS
  )
    throw new Error(
      `invalid_team_worker_count:${workerCount}:expected_1_${GJC_TEAM_MAX_WORKERS}`,
    );
  return {
    workerCount,
    agentType,
