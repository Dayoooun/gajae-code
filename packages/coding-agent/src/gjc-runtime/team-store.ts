...
export type GjcTeamNotificationDeliveryState =
  ...
  | "delivered"
  | "acknowledged";
export type GjcTeamPaneAttemptResult =
  "sent" | "queued" | "deferred" | "failed";
...
export type GjcTeamTaskCompletionEvidenceKind =
  "command" | "inspection" | "artifact";
export type GjcTeamTaskCompletionEvidenceStatus =
  "passed" | "failed" | "not_run" | "verified" | "rejected";
...
const isEnoent = (error: unknown): error is { code: string } =>
...
const taskPath = (dir: string, id: string) =>
  path.join(dir, "tasks", `${safeId("task_id", id)}.json`);
const claimPath = (dir: string, id: string) =>
  path.join(dir, "claims", `${safeId("task_id", id)}.json`);
const writerOptions = (
  filePath: string,
  category: "state" | "prune",
  verb: string,
) => {
...
async function writeJson(filePath: string, value: unknown): Promise<void> {
  await writeJsonAtomic(
    filePath,
    value,
    writerOptions(filePath, "state", "write"),
  );
}
...
export function taskMetadataFromInput(
  ...
  for (const [key, value] of [
    ...
    [
      "required_role",
      optionalString(input.required_role ?? input.requiredRole),
    ],
    [
      "allowed_roles",
      optionalStringArray(input.allowed_roles ?? input.allowedRoles),
    ],
...
export function normalizeGjcTeamTask(raw: GjcTeamTask): GjcTeamTask {
  return {
    ...raw,
    status:
      raw.status === ("complete" as GjcTeamTaskStatus)
        ? "completed"
        : raw.status,
...
function verified(item: GjcTeamTaskCompletionEvidenceItem) {
  return (
    ...
    (item.kind !== "command" && item.status === "verified")
  );
}
export function normalizeGjcTeamTaskCompletionEvidence(
  ...
  if (!isRecord(input) || Array.isArray(input))
    throw new Error(`completion_evidence_required:${id}`);
  if (!Array.isArray(input.items) || !input.items.length)
    throw evidenceError(id, "items");
  const items = input.items.map((value) => {
    ...
    if (kind !== "command" && kind !== "inspection" && kind !== "artifact")
      throw evidenceError(id, "items.kind");
    const status = required(
      id,
      "items.status",
      value.status,
    ) as GjcTeamTaskCompletionEvidenceStatus;
    if (
      (kind === "command" &&
        !["passed", "failed", "not_run"].includes(status)) ||
    ...
    if (kind === "command" && !item.command)
      throw evidenceError(id, "items.command");
    return item;
  });
  if (!items.some(verified))
    throw new Error(`completion_evidence_no_verified_item:${id}`);
...
export function getGjcTeamTaskCompletionEvidenceFailure(
  task: GjcTeamTask,
): string | null {
  ...
  if (!isRecord(evidence) || Array.isArray(evidence))
    return `completion_evidence_required:${task.id}`;
  ...
  try {
    ...
    return error instanceof Error
      ? error.message
      : `invalid_completion_evidence:${task.id}:unknown`;
  }
}
...
export async function readGjcTeamTasksFromDir(
  dir: string,
): Promise<GjcTeamTask[]> {
  try {
...
export async function writeGjcTeamTaskToDir(
  dir: string,
  task: GjcTeamTask,
): Promise<void> {
...
function eligibility(
  task: GjcTeamTask,
  worker: GjcTeamTaskWorker,
  tasks: GjcTeamTask[],
): string | null {
  if (task.status !== "pending") return `task_not_pending:${task.id}`;
  if (task.owner && task.owner !== worker.id)
    return `task_owner_mismatch:${task.id}:${task.owner}`;
  if (task.assignee && task.assignee !== worker.id)
    return `task_assignee_mismatch:${task.id}:${task.assignee}`;
  const roles = new Set(
    [worker.role, worker.agent_type]
  ...
  if (
    task.allowed_roles?.length &&
    !task.allowed_roles.some((role) => roles.has(role))
  )
    return `task_role_mismatch:${task.id}:${task.allowed_roles.join(",")}`;
  if (task.blocked_by?.length)
    return `task_blocked:${task.id}:${task.blocked_by.join(",")}`;
  for (const dependency of task.depends_on ?? [])
    if (
      !tasks.find(
        (candidate) =>
          candidate.id === dependency &&
          isGjcTeamTaskCompletionVerified(candidate),
      )
    )
...
export class GjcTeamTaskStore {
  ...
  async create(
    subject: string,
    description: string,
    options: GjcTeamTaskMetadataInput,
  ) {
  ...
  async claim(
    ...
    const task = id
      ...
      : tasks.find(
          (candidate) => eligibility(candidate, worker, tasks) === null,
        );
    if (!task)
      return {
        ok: false,
    ...
    const existing =
      task.claim ??
      claimRecord(await readJson<unknown>(claimPath(this.dir, task.id)));
    if (existing && !expired(existing.leased_until))
    ...
    if (currentReason || current.status !== "pending") {
      await deleteIfOwned(claimPath(this.dir, task.id), {
        ...
        predicate: (current) =>
          (current as GjcTeamTaskClaim).token === claim.token,
      });
      return {
        ok: false,
  ...
  async transition(
    ...
    const task = await this.read(id);
    if (status === "pending")
      throw new Error(`invalid_task_transition:${id}:pending_requires_release`);
    if (task.status === "completed" || task.status === "failed")
      throw new Error(`task_terminal:${id}`);
    ...
    if (workerId && task.claim.owner !== workerId)
      throw new Error(`claim_owner_mismatch:${id}`);
    ...
    if (terminal)
      await removeFileAudited(
        claimPath(this.dir, id),
        writerOptions(claimPath(this.dir, id), "prune", "terminal"),
      );
