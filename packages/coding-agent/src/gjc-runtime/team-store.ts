...
function claimRecord(value: unknown): GjcTeamTaskClaim | undefined {
	if (
		...
		return undefined;
	return { owner: value.owner, token: value.token, leased_until: value.leased_until };
}
...
export class GjcTeamTaskStore {
	...
	async create(subject: string, description: string, options: GjcTeamTaskMetadataInput) {
		...
		await this.appendEvent({ type: "task_created", task_id: task.id, message: subject });
		return task;
	}
	async update(
		id: string,
		...
		await this.appendEvent({ type: "task_updated", task_id: id, message: updated.subject });
		return updated;
	}
	async claim(worker: GjcTeamTaskWorker, id?: string): Promise<GjcTeamApiClaimResult> {
		...
		if (!task) return { ok: false, reason: id ? `task_not_found:${id}` : "no_pending_task" };
		...
		if (currentReason || current.status !== "pending") {
			...
			return { ok: false, reason: currentReason ?? `task_not_pending:${task.id}` };
		}
		const updated = {
			...current,
		...
		return { ok: true, task: updated, worker_id: worker.id, claim_token: claim.token };
	}
	async transition(id: string, status: GjcTeamTaskStatus, token?: string, workerId?: string, evidenceInput?: unknown) {
		...
		await this.appendEvent({ type: "task_transitioned", task_id: id, message: "Task status changed", data });
		return updated;
	}
