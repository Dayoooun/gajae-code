...
export async function readWorkerStatusFile(
	...
	return (
		(await runtime.readJson<WorkerStatusFile>(statusPath(runtime, dir, worker))) ?? {
			state: "unknown",
...
export async function readWorkerLifecycleRecord(
	...
	const raw = await runtime.readJson<Partial<GjcTeamWorkerLifecycle>>(lifecyclePath(runtime, dir, worker.id));
	const shutdownAck = await runtime.readJson<Record<string, unknown>>(
...
export async function readGjcWorkerHeartbeat(
	...
	const dir = await runtime.findTeamDir(teamName, cwd, env);
	runtime.assertKnownWorker(await runtime.readConfig(dir), worker);
	return runtime.readJson<WorkerHeartbeatFile>(heartbeatPath(runtime, dir, worker));
}
