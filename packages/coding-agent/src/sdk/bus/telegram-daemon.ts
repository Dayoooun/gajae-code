...
interface DaemonOwnerLock {
	pid: number;
	startedAt: number;
}

async function createDaemonOwnerLock(fsImpl: TelegramDaemonFs, file: string, owner: DaemonOwnerLock): Promise<boolean> {
	if (!(await tryOpenWx(fsImpl, file))) return false;
	await fsImpl.writeFile(file, `${JSON.stringify(owner)}\n`, { mode: 0o600 });
	return true;
}

async function daemonOwnerLockIsStale(input: {
	fs: TelegramDaemonFs;
	file: string;
	now: () => number;
	pidAlive: (pid: number) => boolean;
	...
	try {
		const value = JSON.parse(await input.fs.readFile(input.file, "utf8")) as Partial<DaemonOwnerLock>;
		if (!validDaemonPid(value.pid) || typeof value.startedAt !== "number" || !Number.isFinite(value.startedAt))
			return true;
		return input.now() - value.startedAt > HEARTBEAT_TTL_MS && !input.pidAlive(value.pid);
	} catch {
		// An unpublished legacy lock has no owner state to protect. It is recoverable
		// only when no state proves a live owner, which the caller checks separately.
		return true;
	}
}
...
/** Generation-5 records written before fencing lack the acquisition fields and must be replaced, never attached. */
function isLegacyOrUnfencedOwner(state: DaemonState): boolean {
	return (
...
export async function acquireDaemonOwnership(input: {
	...
	const attachDecision = (
		...
		if (state && !hasSafeDaemonStateShape(state)) {
			const malformedPid = (state as { pid?: unknown }).pid;
			if (validDaemonPid(malformedPid) && pidAlive(malformedPid))
		...
		if (state.version !== DAEMON_VERSION || isLegacyOrUnfencedOwner(state))
	...
	if (await createDaemonOwnerLock(fsImpl, paths.lock, { pid, startedAt: now() })) {
	...
	if (
		afterLock &&
		!hasSafeDaemonStateShape(afterLock) &&
		!(await daemonOwnerLockIsStale({ fs: fsImpl, file: paths.lock, now, pidAlive }))
	)
		return { acquired: false, attached: true };
	if (!afterLock && !(await daemonOwnerLockIsStale({ fs: fsImpl, file: paths.lock, now, pidAlive })))
		return { acquired: false, attached: false, provisional: true };
	if (!(await tryOpenWx(fsImpl, paths.steal))) return { acquired: false, attached: false, provisional: true };
	try {
		...
		if (
			...
			!isDefinitelyStoppedOrReusedOwner({ state: rechecked, pidAlive, pidIncarnation })
		) {
			return { acquired: false, attached: false, provisional: true };
		}
		if (
			...
			return { acquired: false, attached: true };
		if (!rechecked && !(await daemonOwnerLockIsStale({ fs: fsImpl, file: paths.lock, now, pidAlive })))
			return { acquired: false, attached: false, provisional: true };
...
export async function renewDaemonHeartbeat(input: {
	...
	ownerId: string;
	acquisitionId?: string;
	/** Bind a heartbeat to the Telegram identity that acquired ownership. */
	tokenFingerprint: string;
	chatId: string;
	fs?: TelegramDaemonFs;
	now?: () => number;
	...
	try {
		...
		const incarnation = input.incarnation ?? defaultTelegramDaemonPidIncarnation(pid);
		const canBindProvisionalPid =
			state?.ownershipPhase === "provisional" && state.launcherPid === state.pid && state.pid !== pid;
		if (
			!state ||
			!hasSafeDaemonStateShape(state) ||
			typeof pid !== "number" ||
...
/** Wait for the exact newly spawned owner to publish a ready state. */
...
export async function releaseDaemonOwnership(input: {
	...
	ownerId: string;
	acquisitionId?: string;
	/** Bind release to the Telegram identity when supplied by current callers. */
	tokenFingerprint: string;
	chatId: string;
	pid: number;
	generation?: number;
...
export async function spawnTelegramDaemonOwner(
	...
	const spawnRuntime = resolveGjcRuntimeSpawnInfo(execPath);
	const ownerId =
		spawnRuntime.mode === "source" && (deps.platform ?? process.platform) === "win32"
			? `daemon-${deps.randomId?.() ?? crypto.randomUUID()}`
			: undefined;
	// The acquisition record owns the launcher PID until the detached child binds
	// its own PID and incarnation.
...
export class TelegramNotificationDaemon {
	...
	requestStop(_reason?: "reload" | "stop" | "signal"): void {
		...
		void this.beginToolActivityShutdown().catch(() => undefined);
	...
	async #drainBtwTurns(): Promise<void> {
		...
		for (const item of this.pool.removeWhere(item => item.payload.btwDelivery !== undefined))
	...
	private async submitThreadedFrame(
		...
		this.submitPool({
			...
			payload: { send, topicLease, ...(toolActivity ? { toolActivity } : {}) },
		});
		await this.flushPool();
	}
	...
	private rememberPendingThreadedFrame(
		...
		this.pendingThreadedFrames.set(session.sessionId, frames);
	}
	...
	private async flushPendingThreadedFrames(sessionId: string, topicLease: TopicAuthorityLease): Promise<void> {
		...
		for (const frame of frames) {
			if (session && session.endpointDigest !== frame.endpointDigest) continue;
			if (frame.msg.type === "tool_activity" && this.opts.toolActivity?.enabled === false) continue;
			await this.submitThreadedFrame(sessionId, frame.send, topicLease, frame.toolActivity);
		}
	}
	...
	private async deliverFlatFallback(
		...
		if (
			!this.submitPool({
				...
				lane: send.lane,
				coalesceKey: send.coalesceKey,
				payload: { send, ...(toolActivity ? { toolActivity } : {}) },
			})
		)
			return;
		await this.flushPool();
