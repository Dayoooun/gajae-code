...
export class AgentSession {
				...
				for (const candidate of candidates) {
					...
					while (true) {
						try {
							...
							const shouldRetry =
								!this.#isAlibabaTokenPlanCompactionTimeout(candidate, message) &&
	...
	#isRetryableError(message: AssistantMessage): boolean {
		if (this.#isAlibabaTokenPlanFirstEventTimeout(message)) return false;
	...
	#classifyErrorForRetry(message: AssistantMessage): RetryErrorClassification {
		...
		if (this.#isAlibabaTokenPlanFirstEventTimeout(message)) return "terminal";
	...
	async #handleManagedAttemptOutcome(outcome: ManagedAttemptOutcome): Promise<ManagedAttemptDecision> {
		...
		if (this.#isAlibabaTokenPlanFirstEventTimeout(outcome.failure.message)) {
			// The managed transport discarded this attempt before session policy saw it.
			// Remove only its provisional controller charge and surface the original message.
			this.#defaultFallbackChain().discardStartedAttempt();
			return {
				type: "terminal",
				terminal: {
