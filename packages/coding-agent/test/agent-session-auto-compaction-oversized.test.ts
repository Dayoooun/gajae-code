...
describe("AgentSession oversized auto-maintenance guard", () => {
	...
	it("does not retry exact Alibaba Token Plan compaction timeout wrappers for the same candidate", async () => {
		...
		const cases = [
			{
				...
				canonical: "OpenAI responses stream timed out while waiting for the first event",
				prefix: "Summarization failed",
			},
			{
				model: completionsModel,
				canonical: "OpenAI completions stream timed out while waiting for the first event",
		...
		for (const testCase of cases) {
			...
			const wrapper = `${testCase.prefix}: ${testCase.canonical}`;
