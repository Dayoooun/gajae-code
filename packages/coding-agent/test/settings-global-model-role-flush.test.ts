...
import { AtomicYamlConflictError } from "../src/config/atomic-yaml-patch";
...
describe("Settings global model role durability", () => {
	...
	it("allows exactly one two-process default-role CAS winner without clobbering the loser", async () => {
		const worker = `
			import { AtomicYamlConflictError, applyAtomicYamlPatches } from ${JSON.stringify(new URL("../src/config/atomic-yaml-patch.ts", import.meta.url).href)};
		...
		const { atomicYamlPathHash } = await import("../src/config/atomic-yaml-patch");
		...
		const results = await Promise.all(
			workers.map(async workerProcess => {
				expect(await workerProcess.exited).toBe(0);
				return JSON.parse(await new Response(workerProcess.stdout).text()) as {
					status: string;
					selector?: string;
					name?: string;
					code?: string;
