...
describe("dev-ci canonical-plan workflow contract", () => {
	...
	test("uses a detached finalized evidence producer and artifact-ID consumer", async () => {
		...
		expect(workflow).toContain("dev-affected-native-${{ github.run_id }}");
		expect(workflow).toContain("telegram-daemon-generation:");
		expect(workflow).toContain("Telegram daemon generation guard");
		expect(workflow).toContain("Require Telegram daemon generation guard");
		expect(workflow).toContain("needs.telegram-daemon-generation.result");
		...
		expect(workflow).toContain("needs: [affected-evidence-producer, affected-plan, affected-native, affected-shards, telegram-daemon-generation, windows-dev-doctor]");
