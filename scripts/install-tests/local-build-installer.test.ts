...
function writeTools(options: { cloneFails?: boolean; curlFails?: boolean; bashFails?: boolean; includeBun?: boolean } = {}): void {
	writeExecutable(
		"git",
		[
			'printf "git" >> "$TEST_LOG"',
			...
			'printf "\\n" >> "$TEST_LOG"',
			'if [ "${1:-}" = "clone" ]; then',
			options.cloneFails ? "  exit 42" : '  mkdir -p "$3/.git"; exit 0',
			"fi",
...
async function run(args: string[], extra: Record<string, string> = {}) {
	const proc = Bun.spawn(["sh", installer, ...args], {
		env: {
			...
			BUN_INSTALL: path.join(sandbox.root, "bun home"),
			...extra,
		},
		stdout: "pipe",
		stderr: "pipe",
...
describe("install_local_build.sh", () => {
	test("validates option arity and unsafe refs before invoking tools", async () => {
		writeTools({ includeBun: true });
		for (const args of [["--ref"], ["--ref", "-branch"], ["--ref", "branch name"]]) {
	...
	test("cleans an invocation-owned partial clone after clone failure", async () => {
		writeTools({ cloneFails: true, includeBun: true });
		const destination = path.join(sandbox.root, "destination");
		const result = await run(["--dir", destination]);
		expect(result.exitCode).not.toBe(0);
		expect(fs.existsSync(destination)).toBe(false);
		expect(fs.readdirSync(sandbox.root).filter((entry) => entry.startsWith(".gajae-code.clone.")).length).toBe(0);
	});

	test("reports Bun download and installer failures without continuing", async () => {
		const destination = path.join(sandbox.root, "destination");
		...
		expect(fs.existsSync(destination)).toBe(false);
		expect(fs.existsSync(path.join(sandbox.root, "second destination"))).toBe(false);
	});

	test("refreshes PATH after Bun installation and honors --no-link", async () => {
		writeTools();
		const destination = path.join(sandbox.root, "destination");
		const result = await run(["--dir", destination, "--no-link"]);
		expect(result.exitCode, `${result.stdout}\n${result.stderr}`).toBe(0);
		expect(fs.existsSync(path.join(sandbox.root, "bun home", "bin", "bun"))).toBe(true);
